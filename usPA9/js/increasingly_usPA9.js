...
if(window.location.href.indexOf('inctesturl') >= 0){
    ...
    function addCSSFile_tick() {
        var cssFilePath = "https://www.increasingly.co/Implementation/usPA9/dev2/css/usPA9.css?v=" + versionUpdate;
    ...
    addCSSFile_tick()
    var jsFilePath = "https://www.increasingly.co/Implementation/usPA9/js/increasingly_usPA9_tickbox.js?v=" + versionUpdate;
    ...
    if (INC_collection_load == true) {
        ...
        function createProductAttObj(childProduct) {
            ...
            // productObj.mainId = ((childProduct != null) && (childProduct != undefined)) ? childProduct["childProductId"] : "";
            ...
            productObj.option[childProduct["optionId"]].imgSrc = (childProduct["childProductImageUrl"] != null) ? childProduct["childProductImageUrl"] : "";
            //optionImageUrl
        ...
        function irbReq(productId, bundleCount, elementName) {
            ...
            //     var bundleResponse = JSON.parse(window.sessionStorage.getItem(productId));
            //     logger(bundleResponse);

            //     if (Math.floor((Date.now() - bundleResponse.timeStamp) / 1000) > 5 * 60) {
            //         return refreshBundles(networkConfigObj, productId)
            //     }

            //     buildDataStoreObj(productId);
            //     // showView();
            //     createPDPBlock();
            //     return
            // }
        ...
        function refreshBundles(networkConfigObj, productId) {
            ...
            // if (count == eleArr.length - 1) {
            if (document.querySelector('.btn.product-form__cart-submit')) {
                if (document.querySelector('.btn.product-form__cart-submit').disabled) {
        ...
        */
        ...
        function getProductIdFromWebpage() {
            ...
            var pid = (el != null) ? el.getAttribute('data-product-id') : "";
            var pid = (el != null) ? el.getAttribute('data-product-id') : "";
            if (document.querySelector('form[action="/cart/add"]') != null) {
                pid = document.querySelector('form[action="/cart/add"]').getAttribute('id').replace('product_form_', '').trim()
            }
            return pid || "";
        }
        ...
        function detectPageType() {
            ...
            if ((el != null) && (el != undefined)) {
                ...
                return;
            } else if (document.querySelector('.template-product') != null) {
                inc.pageType = "pdp";
                logger(inc);
                createPDPBlock();
                setTimeout(function () {
            ...
            el = document.querySelector('.main-content.cart')

            if ((el != null) && (el != undefined)) {
                if (document.querySelector('.empty-page-content') == null) {
        ...
        function get_discount_price_percentage(p_original_price, p_sale_price) {
            return parseInt(((p_original_price - p_sale_price) / p_original_price) * 100);
        }
        ...
        function createSidebarBlock() {
            ...
            // sbBlock.querySelectorAll(getSelectorClassName(sbClassNames, "sidebarCartAddedProductBlock")).forEach(function (block) {
            //     block.remove();
            // });
            ...
            // sbBlock.querySelectorAll(getSelectorClassName(sbClassNames, "productBlock")).forEach(function (block) {
            //     block.remove();
            // });
            ...
            // elToolTip.setAttribute("class", "inc_header_item_count_text_tooltip");
            // elToolTip.style.display = "none";
            // elToolTip.innerText = "Added";
            // elHeaderItemCountTextBlock.appendChild(elToolTip);
            ...
            //siteWidePromo.innerText = "Extra 30% Off Clearance Styles"
            ...
            //sitePromoSpan1.innerHTML = "SAVE 20% OFF $150+ WITH CODE: <span>SAVE20</span>"
            //sitePromoSpan2.innerHTML = "SAVE 25% OFF $200+ WITH CODE: <span>SAVE25</span?";
            //sitePromoSpan3.innerHTML = "Extra 30% off $200+ Use code: <span>GIFT30</span>";
            //sitePromoSpan1.classList.add("PromoSpan1");
            // sitePromoSpan1.style.border ="none"
            ...
            for (var i = 5; i < productListArray.length; i++) {
                ...
                productBlock.querySelector(getSelectorClassName(sbClassNames, "productAddBlock")).addEventListener("click", function (e) {
                    ...
                    logger(qty);
                    logger(findObj);
                    // addProductToCart(findObj, qty, "pdp", this.querySelector(getSelectorClassName(sbClassNames, "productAddText")));
            ...
            sbBlock.querySelector(getSelectorClassName(sbClassNames, "sidebarCheckoutBasketBlock")).addEventListener("click", function () {
                ...
                // elLink.href = "https://uspoloassn.com/cart";
                // this.appendChild(elLink);
                // elLink.click();
                window.location.href = "https://uspoloassn.com/cart";
                // window.open("https://uspoloassn.com/cart", "_blank");
            });

            console.log('running')
            sbBlock.querySelector(getSelectorClassName(sbClassNames, "sidebarCartAddedProductsLeftBtnBlock")).style.opacity = 0.5
            ...
            sbBlock.querySelector(getSelectorClassName(sbClassNames, "sidebarCartAddedProductsRightBtnBlock")).addEventListener("click", function () {
                ...
                // logger("slider btn counter:" + sliderBtnCounter);
                // logger("clicked");
                // logger("product blocks length: " + productBlocks.length);
                ...
                document.querySelector('.inc_cart_added_products_left_btn_block').style.opacity = 1
            });
            ...
            sbBlock.querySelector(getSelectorClassName(sbClassNames, "sidebarCartAddedProductsLeftBtnBlock")).addEventListener("click", function () {
                ...
                if (productBlocks[sliderBtnCounter] != null) {
                    ...
                    document.querySelector('.inc_cart_added_products_right_btn_block').style.opacity = 1
        ...
        function updateSidebarBlock() {
            ...
            elHeaderItemCountText.innerText = Object.keys(incDataStore.bundleCartProducts).length;
            sidebarMobileHeaderItemCountText.innerText = Object.keys(incDataStore.bundleCartProducts).length;
            ...
            if (bundleCartProductsArray.length <= 2) {
                document.querySelector('.inc_cart_added_products_left_btn_block').style.opacity = "0"
                document.querySelector('.inc_cart_added_products_right_btn_block').style.opacity = "0"
            }
        }
        ...
        function showSidebar() {
            ...
            // var cblocked_scrolls1 = setInterval(function(){
            //     if( document.querySelector('body.blocked-scroll') != null){
            //         clearInterval(cblocked_scrolls1)
            //         document.querySelector('body').classList.remove('blocked-scroll')
            //     }
            // })
            ...
            let totalProduct = document.querySelectorAll('.inc_cart_added_products_block .inc_cart_added_product_block').length
        ...
        function addProductToSidebarCart(findObj, qty) {
            var sbClassNames = uiConfig["sidebar"].className;
            var sbBlock = document.querySelector(getSelectorClassName(sbClassNames, "sidebarModalBlock"));
            var sbCartBlock = sbBlock.querySelector(getSelectorClassName(sbClassNames, "sidebarCartAddedProductsBlock"));

            if (sbCartBlock.querySelectorAll('.inc_cart_added_product_block').length <= 2) {
                document.querySelector('.inc_cart_added_products_left_btn_block').style.opacity = "0.5"
                document.querySelector('.inc_cart_added_products_right_btn_block').style.opacity = "1"
            }
        ...
        function createAMBlock(type, PDP_type, recommendation) {
            ...
            // amBlock.querySelectorAll(getSelectorClassName(amClassNames, "productBlock")).forEach(function (block) {
            //     block.remove();
            // });
            ...
            if (PDP_type == "PDP" || recommendation == "recommendation") {
                var cartSection = document.querySelector('#shopify-section-template--14485647687726__product');
                if (window.innerWidth < 768) {
                    let mobilePDP = document.querySelectorAll('.infolinks')[1]
                    insertAfter(mobilePDP, amBlock)
                } else {
                    insertAfter(cartSection, amBlock);
                }
                // trackingEvents("catalogProductView", { "product_id": getProductIdFromWebpage() });
            } else {
                var cartSection = document.querySelector('.bredcrumbWrap')
                insertAfter(cartSection, amBlock)
            }
            ...
            if (amBlock.querySelectorAll('.inc_product_block').length < 4) {
            ...
            // console.log(amBlock.querySelectorAll('.inc_product_block').length)
        ...
        function createPDPBlock() {
            ...
            let pdpLoopCount = bundleProductsArray.length < 4 ? bundleProductsArray.length : 4;
            for (var i = 0; i < pdpLoopCount; i++) {
            ...
            // body.appendChild(pdpBlock);
            //var pdpBlockSibling = document.querySelector('.related-products__title') || document.querySelector('.recommended-products__title') || document.querySelector('body');
            //var pdpBlockSibling_rev = document.querySelector('.shopify-reviews');
            var pdpBlockSibling = document.querySelector('.grid product-single') || document.querySelector('body');
            var pdpBlockSibling_rev = document.querySelector('#shopify-section-product-image-scroll__main');


            if (document.querySelector('.btn.product-form__cart-submit') != null) {
                if (document.querySelector('.btn.product-form__cart-submit').getAttribute('disabled') == "disabled") {
                    if (pdpBlock != null) {
            ...
            if (document.querySelector('.grid.product-single') != null) {
                if (window.innerWidth < 768) {
                    let mobilePDP = document.querySelectorAll('.infolinks')[1]
                    insertAfter(mobilePDP, pdpBlock)
                } else {

                    insertAfter(document.querySelector('.grid.product-single'), pdpBlock)
                }
            ...
            let clientButton = document.querySelector('.btn.product-form__cart-submit')
            if (clientButton.disabled) {
                pdpBlock.style.display = "none"
            }
            ...
            var elProductAddBlock = document.querySelectorAll(getSelectorClassName(pdpClassNames, "productAddBlock"));

            for (var i = 0; i < 3; i++) {
                if (elProductAddBlock[i]) {
                    elProductAddBlock[i].click();
                }
            }

            // setInterval(() => {
            //     if(incDataStore.bundleCartProducts != undefined){
            //         logger("incDataStore bundleCart Products");
            //         logger(incDataStore.bundleCartProducts);
            //     }
            // }, 3000);
            ...
            pdpBlock.querySelector(getSelectorClassName(pdpClassNames, "pdpBundleProductRightBtnBlock")).addEventListener("click", function () {
                ...
                if (sliderBtnCounter + prod_c == productBlocks.length - 1) {
            ...
            if (bundleProductsArray.length <= 2 && window.innerWidth > 1189) {
                pdpBlock.querySelector(getSelectorClassName(pdpClassNames, "pdpBundleProductRightBtnBlock")).setAttribute("style", "pointer-events: none; opacity: 0");
                pdpBlock.querySelector(getSelectorClassName(pdpClassNames, "pdpBundleProductRightBtnImgBlock")).setAttribute("style", "opacity: 0");
                pdpBlock.querySelector(getSelectorClassName(pdpClassNames, "pdpBundleProductLeftBtnBlock")).setAttribute("style", "pointer-events: none; opacity: 0");
                pdpBlock.querySelector(getSelectorClassName(pdpClassNames, "pdpBundleProductLeftBtnImgBlock")).setAttribute("style", "opacity: 0");
            }
            ...
            // pdpBlock.addEventListener("click", function(){
            //     hideProductAttBlock();
            // }, false);
            ...
            // trackingEvents("productPageVisit", { "product_id": getProductIdFromWebpage() });
            // trackingEvents("catalogProductView", { "product_id": getProductIdFromWebpage() });

            var clientSwatchOptions = document.querySelectorAll('.swatch.clearfix')
            if (clientSwatchOptions != null) {
                clientSwatchOptions.forEach(function (element) {
            ...
            var elClientAddToCartBtn = document.querySelector('.btn.product-form__cart-submit')
            elClientAddToCartBtn.addEventListener("click", function (event) {
                ...
                setTimeout(function () {
                    ...
                    if ((warningPara == null) || (warningPara == undefined)) {
                        ...
                        var clientQtyBlock = document.querySelector(".product-single");
                        var productQty = clientQtyBlock.querySelector(".product-form__input.qty").value;
                        addProductToSidebarCart(findObj, productQty);
                        showSidebar();
                        sbBlock.querySelector(getSelectorClassName(uiConfig["sidebar"].className, "sidebarCartAddedProductsLeftBtnBlock")).style.opacity = 0
                        sbBlock.querySelector(getSelectorClassName(uiConfig["sidebar"].className, "sidebarCartAddedProductsRightBtnBlock")).style.opacity = 0
                        let timeClientPopup = setInterval(() => {
                            let clientPopup = document.querySelector('.mfp-close')
                            if (clientPopup) {
                                clientPopup.click()
                                clearInterval(timeClientPopup)
                            }
                        }, 1);
                        setTimeout(() => {
                            clearInterval(timeClientPopup)
        ...
        function navigateToProductPage(productBlock) {
        ...
        */
        function createProductBlock(childProductId, blockType) {
            ...
            productLink.addEventListener('click', function (e) {
                ...
                e.preventDefault()
                hideProductAttBlock();
                navigateToProductPage(productBlock);
            ...
            elTitleBlock.addEventListener("click", function (e) {
                e.preventDefault()
                navigateToProductPage(productBlock);
                // trackingEvents("bundleProductClickTracking", { "product_id": productBlock.getAttribute("data-main_id"), "core_product_id": getProductIdFromWebpage()});
                // window.location.href = productObj.url;
            });
            ...
            if (save_prc != 0) {
                ...
                elworthPrice.innerHTML = "Save " + currencySymbol + parseFloat(worthPrice).toFixed(2)
                var elworthPriceSpanPrice = document.createElement('span');
                elworthPriceSpanPrice.innerHTML = " " + currencyName + " (" + save_prc + "% off)"
            ...
            if ((productObj.ProductLevelDiscountMessage != null || productObj.ProductLevelDiscountMessage != undefined) && elPromoText != undefined) {
                ...
                elPromoBlock.classList.add('inc_active');
                elPromoText.innerHTML = "Extra 20% off $100+  Use code: GIFT20 <br> Extra 25% off $150+ Use code: GIFT25 <br> Extra 30% off $200+ Use code: GIFT30";
                elPromoText.style.textTransform = "none"
            ...
            elAttBlock.addEventListener('click', function () {
                ...
                elAttSel.setAttribute('inc_open', true)
            });
            ...
            // var elAddInput = document.createElement('input');
            // elAddInput.setAttribute('type', 'checkbox');

            // elProductAddImg.appendChild(elAddInput);
            ...
            var productColorsObj = []
            if (productObj["option"] != null) {
                productColorsObj = getProductColorsObj(findObj);
                ...
                productColorsObj["colorArray"].forEach(function (colorObj) {
                    ...
                    attSelColorImgBlock.addEventListener("click", function () {
                        ...
                        elAttSelBlock.setAttribute("data-value", colorText.toUpperCase());
                        // logger(this);
                        ...
                        if (productSizesObj["sizeArray"].length > 0) {
                            productSizesObj["sizeArray"].forEach(function (sizeObj) {
                                ...
                                attSelSizeImgBlock.addEventListener("click", function () {
                                    ...
                                    elAttSelBlock.setAttribute("data-value", sizeText.toUpperCase());
                                    // logger("parentnode");
                                    ...
                                    this.querySelector(getSelectorClassName(classNames, "productAttSelSizeImg")).classList.add('inc_active')

                                    let that = this
                                    productBlock.querySelectorAll('.inc_product_att_sel_size_img').forEach(function (element) {

                                        if (element.parentNode.getAttribute('data-size') != that.getAttribute('data-size')) {

                                            element.classList.remove('inc_active')
                                        }
                                    })
                                    // update sku;
                                    findObj = new FindObj();
                                    ...
                                    if (productLengthsObj["lengthArray"].length > 0) {
                                        ...
                                        productLengthsObj["lengthArray"].forEach(function (lengthObj) {
                                            ...
                                            attSelLengthImgBlock.addEventListener("click", function () {
                                                ...
                                                this.querySelector('.inc_product_att_sel_length_img').classList.add('inc_active')
                                                let that = this
                                                productBlock.querySelectorAll('.inc_product_att_sel_length_img').forEach(function (element) {

                                                    if (element.parentNode.getAttribute('data-length') != that.getAttribute('data-length')) {
                                ...
                                attSelSizeImgBlock.setAttribute("data-value", sizeObj["text"].toUpperCase());
                                attSelSizeImgBlock.setAttribute('title', sizeObj["text"]);
                    ...
                    // attSelColorImg.style.backgroundColor = productsColorObj[productSKU].text;
                    // if (productsColorObj[productSKU].imgSrc != null &&
                    //     productsColorObj[productSKU].imgSrc != undefined &&
                    //     productsColorObj[productSKU].imgSrc != "") {
                    //     attSelColorImg.style.backgroundImage = "url(" + productsColorObj[productSKU].imgSrc + ")";
                    // }
                    // attSelColorImg.style.backgroundColor = colorObj["text"];
                    ...
                    attSelColorImgBlock.setAttribute('data-value', colorObj["text"].toUpperCase());
                    ...
                    // if((inc.deviceType != null) && (inc.deviceType != undefined) && (inc.deviceType == "desktop")){
                    //     attSelColorImgBlock.addEventListener("mouseover", function(){
                    //         // logger("mouseover" + colorObj["text"]);
                    //         attSelColorTooltipDiv.style.display = "block"; 
                    //     });
                    //     attSelColorImgBlock.addEventListener("mouseout", function(){
                    //         // logger("mouseout");
                    //         attSelColorTooltipDiv.style.display = "none";
                    //     });
                    // }
            ...
            // logger(findObj);
            // logger(getProductSizesObj(findObj));
                    ...
                    Object.keys(productObj["option"]).forEach(function (option) {
                        ...
                        switch (option) {
                            case "1":
                                ...
                                // elAttColorImg.style.backgroundImage = "url(" + productObj["option"]["1"]["imgSrc"] + ")";
                                elAttSelColorTitleText.innerHTML = productObj["option"]["1"].label.toUpperCase() + ": <span>" + productObj["option"]["1"]["text"] + "</span>"
                                break;
                            case "2":
                                elAttSizeTitleText.innerText = productObj["option"]["2"]["label"].toUpperCase();
                                elAttSizeImg.innerText = productObj["option"]["2"]["text"];
                                elAttSelSizeTitleText.innerHTML = productObj["option"]["2"].label.toUpperCase() + ":  <span>" + productObj["option"]["2"]["text"] + "</span>"
                                break;
                            case "3":
                                elAttLengthTitleText.innerText = productObj["option"]["3"]["label"].toUpperCase();
                                elAttLengthImg.innerText = productObj["option"]["3"]["text"];
        ...
        */
        ...
        function clientProductAutoSelect(type) {
            setTimeout(() => {
                let clientColor = undefined
                let clientSize = undefined
                let clientLength = undefined

                clientColor = document.querySelectorAll('.slVariant')[0]
                clientSize = document.querySelectorAll('.slVariant')[1]
                clientLength = document.querySelectorAll('.slVariant')[2]


                if (clientColor !== undefined) {

                    let foundColor = document.querySelector('.inc_pdp_product-main_block .inc_product_att_sel_color_img_block[data-value="' + clientColor.innerText + '"]')
                    if (foundColor) foundColor.click()
                }
                if (clientSize !== undefined) {

                    let foundColor = document.querySelector('.inc_pdp_product-main_block .inc_product_att_sel_size_img_block[data-value="' + clientSize.innerText.toUpperCase() + '"]')
                    if (foundColor) foundColor.click()
                }

                if (clientLength !== undefined) {

                    let foundColor = document.querySelector('.inc_pdp_product-main_block .inc_product_att_sel_length_list_block .inc_product_att_sel_length_img_block[data-length="' + clientLength.innerText.toUpperCase() + '"]')
                    if (foundColor) foundColor.click()
                }
        ...
        function getProductByAttFromClient() {
            ...
            // for (children of swatchOptions.children) {
            //     children.querySelectorAll("div").forEach(function (childDiv) {
            //         if (childDiv.getAttribute("data-id") != null) {
            //             logger(childDiv);
            //             logger(childDiv.style.borderColor);
            //         }
            //     });
            // }
        ...
        */
        function getProductColorsObj(findObj) {
            ...
            if (findObj.hasOwnProperty("mainId")) {
                for (mainProductId in productListObj) {
                    if (mainProductId == findObj["mainId"]) {
                        for (childProductId in productListObj[mainProductId]) {
                            // productColorObj[childProductId] = {};
                            // productColorObj[childProductId]["imgSrc"] = productListObj[mainProductId][childProductId]["option"]["1"]["imgSrc"];
                            // productColorObj[childProductId]["text"] = productListObj[mainProductId][childProductId]["option"]["1"]["text"];
                            // productColorObj[productListObj[mainProductId][childProductId]["option"]["1"]["text"]] = productListObj[mainProductId][childProductId]["option"]["1"]["imgSrc"];
                            // var colorObj = {};
                            // productColorArrayObj[productListObj[mainProductId][childProductId]["option"]["1"]["text"]] = productListObj[mainProductId][childProductId]["option"]["1"]["imgSrc"];
            ...
            // logger(productColorArrayObj);
            // logger(productColorObj);
            // logger(productColorObjArray);
            // return productColorObj;
            return productColorObj;
            // return productColorObjArray;
        }
        ...
        */
        function getProductSizesObj(findObj) {
            ...
            var sizesArray = ["S", "M", "L", "XL", "XX", "XXL", "2XL"];
        ...
        */
        function calculateBundleTotal() {
            ...
            try {
                ...
                elPriceWorthText.innerHTML = "Save: " + currencySymbol + parseFloat(worthPrice).toFixed(2) + " " + currencyName
                var elworthCartPriceSpanPrice = document.createElement('span');
                elworthCartPriceSpanPrice.innerHTML = " (" + save_prc + "% Off)"
                elPriceWorthText.appendChild(elworthCartPriceSpanPrice);



                elPriceAddonTitleText.innerText = totalAddonProductsCount + " ADDON SELECTED"
        ...
        */
        function getProductByAtt(findObj) {
            ...
            for (childProductId in mainProductListObj) {
                // logger(mainProductListObj[childProductId]["option"]["1"].text);
                // logger(mainProductListObj[childProductId]["option"]["2"].text);
                // logger((findObj.colorText == mainProductListObj[childProductId]["option"]["1"].text) &&
        ...
        */
        ...
        //         for (mainProductId in productListObj) {
        //             for (childProductId in productListObj[mainProductId]) {
        //                 // logger(childProductId == findObj["id"]);
        //                 if (childProductId == findObj["id"]) {
        ...
        */
        ...
        //     // formData.append('option-0', 'TRIBECA GRAY 2373');
        //     // formData.append('option-1', 29);
        //     // formData.append('option-2', 30);
        ...
        //     // formData1.append('id', '16148271267886');
        //     // formData1.append('option-0', 'Blue-19867i');
        //     // formData1.append('option-1', 29);
        //     // formData1.append('option-2', 30);
        //     // formData1.append('quantity', 2);

        //     // networkReqForCart(reqURL, formData, 'POST', function () {
        //     //     var formData = new FormData();
        //     //     formData.append('form_type', 'product');
        //     //     formData.append('id', '16148568735790');
        //     //     formData.append('option-0', 'TRIBECA GRAY 2373');
        //     //     formData.append('option-1', 29);
        //     //     formData.append('option-2', 30);
        ...
        //     //         var formData = new FormData();
        //     //         formData.append('form_type', 'product');
        //     //         formData.append('id', '16340385464366');
        //     //         formData.append('option-0', 'HARVEST PLUM');
        //     //         formData.append('option-1', 30);
        //     //         formData.append('option-2', 32);
        //     //         formData.append('quantity', 1);
        ...
        //     //     });
        //     // });
        //     // networkReq(reqURL, formData1, 'POST');
        // }
        ...
        function addProductToCart(findObj, qty, pageType, productBlock) {
            ...
            if ((pageType != "cartPage")) {
                ...
                setTimeout(function () {
                    ...
                    productBlock.remove()
                    console.log("2")
                    if (document.querySelector('.inc_recommend_items_listing_products_list_block').innerHTML == "") {
                        window.location.href = "https://uspoloassn.com/cart";
                    }
                }, 200);

                addProductToSidebarCart(findObj, qty);
                trackingEvents("bundleAddToCart", eventDataObj, (inc.pageType == "productList") ? "101" : "107");
            }
        ...
        */
        ...
        function trackingEvents(eventType, eventDataObj, pageType) {
            ...
            switch (eventType) {
                ...
                case "bundleProductClickTracking":
                    ...
                    if (
                        (INC.config.recommendation == true && INC.config.recommendationProduct.includes(eventDataObj.product_id)) && inc.pageType == "pdp") {
        ...
        function clickListener(e) {
            ...
            if (clickedElement.className == "close" || clickedElement.parentNode.parentNode.className == "close") {
                ...
                //console.log(productBlocks.length)

            }

            if (clickedElement.className !== "inc_product_att_sel_done_btn_block" && clickedElement.className !== "inc_product_desc_att_color_img"
                && clickedElement.className !== "inc_product_desc_att_color_img"
                && clickedElement.className !== "inc_product_desc_att_size_title_text"
                && clickedElement.className !== "inc_product_desc_att_color_title_text"
                && clickedElement.className !== "inc_product_desc_att_size_img_block"
                && clickedElement.className !== "inc_product_desc_att_color_img_block"
                && clickedElement.className !== "inc_product_att_sel_color_list_block"
                && clickedElement.className !== "inc_product_att_sel_size_img"
                && clickedElement.className !== "inc_product_att_sel_size_title_text"
                && clickedElement.className !== "inc_product_att_sel_color_img"
                && clickedElement.className !== "inc_product_att_sel_size_img_block"
                && clickedElement.className !== "inc_product_att_sel_color_title_text"
                && clickedElement.className !== "inc_product_att_sel_size_block"
                && clickedElement.className !== "inc_product_att_sel_size_img inc_active"
                && clickedElement.className !== "inc_product_att_sel_done_btn_text inc_active"
                && clickedElement.className !== "inc_product_att_sel_done_btn_block"
                && clickedElement.className !== "inc_product_att_sel_size_list_block"
                && clickedElement.className !== "inc_product_desc_att_size_img"
                && clickedElement.className !== "inc_product_att_sel_done_block"
                && clickedElement.className !== "inc_product_desc_att_length_img"
                && clickedElement.className !== "inc_product_desc_att_length_title_text"
                && clickedElement.className !== "inc_product_desc_att_length_img_block"
                && clickedElement.className !== "inc_product_att_sel_length_img_block"
                && clickedElement.className !== "inc_product_att_sel_length_img inc_active"
                && clickedElement.className !== "") {

                document.querySelectorAll('.inc_product_att_sel_block').forEach(function (element) { element.style.display = "none" })
            }
    ...
    function incPriceMismatch(price, special_price) {
        ...
        if (document.querySelector('.product-price__sale') != null) {
            if (document.querySelector('.product-price__sale').innerText.replace(',', '').replace('$', '').split(" ")[0].trim() != null) {
                priceMismatchObj.special_price = document.querySelector('.product-price__sale').innerText.replace(',', '').replace('$', '').replace('US', '').split(" ")[0].trim()
                //priceMismatchObj.special_price = priceMismatchObj.special_price.replace(' ', '')
            }

        }
        if (document.querySelector('.product-price__price') != null) {
            if (document.querySelector('.product-price__price').innerText.replace(',', '').replace('$', '').split(" ")[0].trim() != null) {
                priceMismatchObj.product_price = document.querySelector('.product-price__price').innerText.replace(',', '').replace('$', '').replace('US', '').split(" ")[0].trim()
    ...
    if (true) {
        ...
        function checkScrollDirection(event) {
            if (document.querySelector(".inc_pdp_block") != null) {
                ...
                if (inc_isOnScreen_page(document.querySelector('.inc_pdp_block')) && is_visible_elem_counter < 1) {
                    trackingEvents("catalogProductView", { "product_id": getProductIdFromWebpage() });
                    is_visible_elem_counter++;
                }
            } else if (document.querySelector(".inc_am_block") != null && inc.pageType == "pdp") {
                var inc_div = document.querySelector(".inc_am_block");
                if (inc_isOnScreen_page(document.querySelector('.inc_am_block')) && is_visible_elem_counter < 1) {
                    trackingEvents("catalogProductView", { "product_id": getProductIdFromWebpage() });
                    is_visible_elem_counter++;
                }
            }
