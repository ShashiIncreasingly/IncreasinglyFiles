...
(function(){
    ...
    INC.init = function () {
        ...
        var paramdata = ""
        if(INC.config.ivid != ""){
            paramdata = { "eventData": btoa((JSON.stringify({"vid":INC.config.ivid,"token":"C1iNq5"}))) };
        }else{
            paramdata = { "eventData": btoa((JSON.stringify({"token": 'C1iNq5'}))) };
        }
            ...
            if (xhr.readyState == XMLHttpRequest.DONE) {
                if (xhr.status == 200) {
                    if (xhr.responseText != "" && xhr.responseText != null) {
                        ...
                        INC.config.noimage = resData.increasinglyFilesDomain + "https://www.increasingly.co/Implementation/p3ThM9/images/noimage.png"
        ...
        if (clickedElement != null) {
            try {
                ...
                if(clickedElement != null){
                    ...
                    if(INC.config.pageType == "pdp"){
                        if(clickedElement.classList.contains('js-add-to-bag-button') == trueflag && clickedElement.getAttribute('data-test-id') == "add_to_bag_btn" || clickedElement.classList.contains('js-btn-add-to-bag') == trueflag && clickedElement.getAttribute('data-test-id') == "add-to-cart"){
                            var findObj = new INC.classes.FindObj();
                            var p_ids = INC.methods.getProductIdFromWebPage()
                        ...
                        if(clickedElement.parentNode != null){
                            if(clickedElement.classList.contains('js-shade-picker-item-name') == trueflag || clickedElement.classList.contains('js-shade-swatch--image-wrapper') == trueflag || clickedElement.classList.contains('elc-shade-picker-item--selected') == trueflag || clickedElement.classList.contains('js-dropdown-option') == trueflag || clickedElement.parentNode.parentNode.classList.contains('sells-with-grid') == trueflag || clickedElement.parentNode.classList.contains('sells-with-grid') == trueflag || clickedElement.classList.contains('sells-with-grid') == trueflag){
                    ...
                    if (INC.config.pageType == "productList") {
                        if(clickedElement.classList.contains('js-add-to-bag-button') == trueflag){
