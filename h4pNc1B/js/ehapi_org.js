...
(function(){
    ...
    if (window.location.hostname == 'tt311.w2solution.com'){
        ...
        INC.config.addToBasketURL = "https://cefinecosmetics.co.jp/Form/Product/ProductDetail.aspx?shop=0&";
    }
    if (window.location.hostname == 'tt311.w2solution.com'){
        INC.clientConfig.checkoutLink = "https://tt311.w2solution.com/Test/R1024_Cefine/Form/Order/CartList.aspx";
    } else {
        INC.clientConfig.checkoutLink = "https://cefinecosmetics.co.jp/Form/Order/CartList.aspx";
    }
    ...
    INC.methods.getProductIdFromWebPage = function () {
        ...
        product_id = pid.split('pid=')[1].trim()
        if(product_id.indexOf('&cat') > 0){
            product_id.split('&cat')[0].trim()
        }
        INC.config.Product_id = product_id.split('&vid')[0]
        return product_id.split('&vid')[0];
    }

    INC.methods.detectPageType = function () {
        if (document.querySelector('#dvProductDetailArea') != null) {
            return "pdp";
        }
        if(window.location.href.indexOf('Form/Order/CartList') >= 0) {
            return "cartPage";
        }
    }
        ...
        if (sidebar != "sidebar") {
            if (INC.dataStore.dataStoreObj.undefined != undefined || empty_sidebarflag == falseflag) {
                ...
                if (window.innerWidth > 1139){
                    pdpBlockSibling = document.querySelector('#detailThree')
                } else {
                ...
                if (window.innerWidth > 800) {
                    var g_src_img = pdpBundleProductListMainBlock.querySelector('.inc_product_img_main_img img').src
                    var g_href_a = pdpBundleProductListMainBlock.querySelector('.inc_product_img_main_img a').href
