...
(function(){
    ...
    INC.config.FBTTitle = "RECOMMEND TO USE TOGETHER"
    ...
    function addCSSFile() {
        var cssFilePath = "https://jp.increasingly.co/Implementation/Ce5i9e/css/Ce5i9e.css";
    ...
    INC.init = function () {
        ...
        switch (INC.config.pageType) {
            case "pdp":
                ...
                break;
            case "cartPage":
                Array.prototype.forEach.call(document.querySelectorAll('.list .product .item_img a'), function(elProductCode) {
                    var cartId = elProductCode.href.split('&pid=')[1].split('&')[0]
                    INC.config.cartproductids.push(cartId.split('.')[0]);
                });
                        ...
                        for (var ir in irbProductsArray) {
                            if(Object.prototype.hasOwnProperty.call(irbProductsArray, ir)){
                                ...
                                if(INC.config.datastoreprod.indexOf(product.ProductId) == -1){
                                    ...
                                    //add this code for 2 attributes - merged both attribute
                                    if (pAttributes.length > 1) {
                                                        ...
                                                        for (var vS in atvalues) {
                                                            ...
                                                            if (Object.prototype.hasOwnProperty.call(atvalues, vS)) {
                                                                if (atvaluesfirst[c].childProductId == atvalues[vS].childProductId) {
                                                                    atvaluesfirst[c].option_text_ =  atvaluesfirst[c].optionText + " / " + atvalues[vS].optionText
                                                                }
                                                            }
                                                        }
                                                    }
    ...
    INC.methods.createPDPBlock = function (sidebar) {
        ...
        if (sidebar != "sidebar") {
            if (INC.dataStore.dataStoreObj.undefined != undefined || empty_sidebarflag == falseflag) {
                ...
                var modalTitle1 = pdpBlock.querySelector('.inc_pdp_title_text1');
                modalTitle.innerText = INC.config.FBTTitle
                modalTitle1.innerText = INC.config.FBTTitleJp
                ...
                if (window.innerWidth > 1139){
                    ...
                    pdpBlockSibling = document.querySelector('.description')
                }
    ...
    INC.methods.createProductShowcaseBlock = function (pdpProductId, blockType, _bundleProductListCheck, _parentblock,elProductShowcaseBlock,mainprod) {
        ...
        switch (blockType) {
            ...
            case "sidebar": elAddText.textContent = "まとめてカートに入れる"; break;
            case "am": elAddText.textContent = "まとめてカートに入れる"; break;
        }
                        ...
                        if (window.innerWidth > 820){
                            if (elAddText.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.className == 'inc_pdp_bundle_product_list_main_block'){
                                if (document.querySelector('.inc_goes_img_block') != null){
                                    document.querySelector('.inc_goes_img_block').style.display = "block"
                                }
                        ...
                        if (prod_card.querySelector('.inc_product_modal_block.inc_active') == null) {
                            ...
                            if(window.innerWidth > 820){
                                if (elAddText.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.className == 'inc_pdp_bundle_product_list_main_block'){
                                    ...
                                    document.querySelector('.inc_goes_img_block').style.display = "none"
    ...
    INC.methods.addBundleToCart = function (btn_cart) {
        ...
        Object.keys(dataStore.bundleCartProducts).forEach(function (bundleCartProduct) {
            ...
            var productObj = INC.dataStore.methods().getProductById(findObj);
            var var_selectedid= productObj.childsku
            if (window.location.hostname == 'tt311.w2solution.com'){
                var addProductURL = productObj.url.replace('https://cefinecosmetics.co.jp/Form/Product/ProductDetail.aspx?','https://tt311.w2solution.com/Test/R1024_Cefine/Form/Product/ProductDetail.aspx?')
            } else {
                var addProductURL = productObj.url
            }
            
            if(addProductURL.indexOf('vid') == -1){
            	// addProductURL = addProductURL + "&vid="+var_selectedid+""
            }
            //text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9
            var param_=""
            var datac = ""
            var type_v = productObj.ProductType
            window.jQuery.ajax({
                type: "GET",
                url: addProductURL,
                headers: {
                ...
                success: function(html_data) {
                    ...
                    var simpleprodel = document.createElement('div')
                    simpleprodel.innerHTML=html_data
                 ...
                 //    if(INC.config.Product_id == productObj.mainId){
	                ...
	                //     var elm = document.querySelector('#aspnetForm')
                	// }else{
                	// 	var d__VIEWSTATE = simpleprodel.querySelector('form[name="aspnetForm"] #__VIEWSTATE').value
	                //     var d__EVENTVALIDATION = simpleprodel.querySelector('form[name="aspnetForm"] #__EVENTVALIDATION').value
	                //     var elm = simpleprodel.querySelector('#aspnetForm')
                	// }
                	var d__VIEWSTATE = simpleprodel.querySelector('form[name="aspnetForm"] #__VIEWSTATE').value
	                    var d__EVENTVALIDATION = simpleprodel.querySelector('form[name="aspnetForm"] #__EVENTVALIDATION').value
	                    var elm = simpleprodel.querySelector('#aspnetForm')
                    
                    var checktest=/^(text|password|hidden|search|tel|url|email|number|range|color|datetime|date|month|week|time|datetime-local)$/i
                    var f = true;
                    window._isCrossPost = false;
                    var w = elm;
                    if (window._activeDefaultButton && !window._activeDefaultButtonClicked)
                        window._onFormElementActive(window._activeDefaultButton, 0, 0);
                    var b = new Sys.StringBuilder
                    , s = w.elements
                    , B = s.length
                    , t = '';
                    b.append(t);
                    if (document.querySelector('.productCart .btnCart a') != null){
                        var str=document.querySelector('.productCart .btnCart a').id
                        var strsp = str.split('1_')[1]
                    } else {
                        var strsp = 'lbCartAdd'
                    }
                    ...
                    for (f = 0; f < B; f++) {
                        if(f == 0){
                            b.append(encodeURIComponent("ctl00$smScriptManager"));
                            b.append("=");
                            b.append(encodeURIComponent("ctl00$ContentPlaceHolder1$upUpdatePanel|ctl00$ContentPlaceHolder1$"+strsp+""));
                            b.append("&")
                        }
                        var e = s[f]
                        , g = e.name;
                        if (typeof g === "undefined" || g === null || g.length === 0 || g === window._scriptManagerID)
                            continue;
                        ...
                        if (n === "INPUT") {
                            ...
                            var viewstate = ""
                            var evtvalidation = ""
                            viewstate = d__VIEWSTATE
                            evtvalidation = d__EVENTVALIDATION
                            if(g == '__VIEWSTATE'){
                                b.append(encodeURIComponent(g));
                            b.append("=");
                            b.append(encodeURIComponent(viewstate));
                            b.append("&")
                            }else if(g == '__EVENTVALIDATION'){
                            b.append(encodeURIComponent(g));
                            b.append("=");
                            b.append(encodeURIComponent(evtvalidation));
                            b.append("&")
                            }else if(g == '__EVENTTARGET'){
                            b.append(encodeURIComponent(g));
                            b.append("=");
                            
                            b.append(encodeURIComponent('ctl00$ContentPlaceHolder1$'+strsp+''));
                            b.append("&")
                            }else if (checktest.test(p) || (p === "checkbox" || p === "radio") && e.checked) {
                            b.append(encodeURIComponent(g));
                            b.append("=");
                            ...
                            if(type_v == "configurable"){
                                for (var q = 0; q < A; q++) {
                                    var u = e.options[q];
                                    if (u.selected) {
                                        if(g == 'ctl00$ContentPlaceHolder1$ddlVariationSelect'){
                                            b.append(encodeURIComponent(g));
                                            b.append("=");
                                            b.append(encodeURIComponent(var_selectedid));
                                            b.append("&")
                                        }else{
                                            b.append(encodeURIComponent(g));
                                            b.append("=");
                    ...
                    if(document.querySelector('select[name="ctl00$ContentPlaceHolder1$ddlVariationSelect"]') == null){
                        if(type_v == "configurable"){
                            b.append(encodeURIComponent("ctl00$ContentPlaceHolder1$ddlVariationSelect"));
                            b.append("=");
                            b.append(encodeURIComponent(var_selectedid));
                            b.append("&")
                        }
                    ...
                    if (window._additionalInput) {
                        b.append(window._additionalInput);
                        window._additionalInput = null
                    }
                    var c = new Sys.Net.WebRequest
                    c.set_url(addProductURL);
                    c.get_headers()["X-MicrosoftAjax"] = "Delta=true";
                    c.get_headers()["Cache-Control"] = "no-cache";
                    c.set_timeout(window._asyncPostBackTimeout);
                    c.add_completed(Function.createDelegate(window, window._onFormSubmitCompleted));
