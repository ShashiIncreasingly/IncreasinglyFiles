...
INC.classes.ProductObj = function (product) {
    ...
    if(this.promoField == "" || this.promoField == undefined){
        this.promoField = ((product != null) && (product != undefined)) ? product["PromotionalMessage"] : "";
    }
}
...
INC.methods.createProductShowcaseBlock = function (pdpProductId, blockType, bundleProductListCheck, parentblock) {
    ...
    if (productBlock.querySelector('.pc_promo_div') == null && productObj.ProductLevelDiscountMessage != null && productObj.ProductLevelDiscountMessage != "") {
        ...
        if(productObj.promoField != undefined){
            if (productObj.promoField.length >= 25){  
                ...
                let additional_div = generate_html_tag('div', 'inc_promo_span_superparent_')
                let additional_chlid_div = generate_html_tag('div', 'inc_avail_offer_text')
                additional_chlid_div.innerHTML = "Offers Available"
                additional_div.appendChild(additional_chlid_div)
                let parent_promo_div = generate_html_tag('div', 'inc_promo_span_parent')
                let parent_promo_close_div = generate_html_tag('div', 'inc_promo_span_close_parent')
                parent_promo_div.appendChild(parent_promo_close_div)
                additional_div.appendChild(parent_promo_div)
                parent_promo_div.appendChild(ec_promo_span)
                ec_promo_div.appendChild(additional_div);
                additional_chlid_div.addEventListener('click',function(e){
                    ec_promo_div.classList.add("inc_promo_open");
                });
        
                parent_promo_close_div.addEventListener('click',function(e){
                    ec_promo_div.classList.remove("inc_promo_open");
                });
            } else {
                ec_promo_div.classList.add('noOnHover')
                ec_promo_span.innerHTML = productObj.ProductLevelDiscountMessage.split('|')[0].replace("_", " ")
                ec_promo_span.setAttribute('title', productObj.ProductLevelDiscountMessage.split('|')[0].replace("_", " "))
        ...
        productBlock.classList.add('inc_promo_present')
...
INC.methods.updateProductBlock = function (productBlock, findObj, blockType, bundleProductListCheck) {
    ...
    if (Object.keys(productObj).length != 0) {
        ...
        if (productObj.ProductLevelDiscountMessage != null && productObj.ProductLevelDiscountMessage != "") {
            if(productBlock.querySelector('.pc_promo_div')){
                if(productBlock.querySelector('.pc_promo_div').classList.contains('empty')){
                    productBlock.querySelector('.pc_promo_div').remove()
                }
                if(productBlock.querySelector('.pc_promo_div')){
                    productBlock.querySelector('.pc_promo_div').remove()
                }
            ...
            ec_promo_span.classList.add('red_promo')
            if (productObj.promoField != undefined){  
                if (productObj.promoField.length >= 25 && productBlock.parentElement.className == 'inc_product_showcase_block'){  
                    ec_promo_span.innerHTML = productObj.ProductLevelDiscountMessage.split('|')[0].replace("_", " ")
                    ec_promo_span.setAttribute('title', productObj.ProductLevelDiscountMessage.split('|')[0].replace("_", " "))
                    let additional_div = generate_html_tag('div', 'inc_promo_span_superparent_')
                    let additional_chlid_div = generate_html_tag('div', 'inc_avail_offer_text')
            ...
            if (document.querySelector('.pc_promo_div') == null){
                var ec_promo_div = document.createElement('div');
                ec_promo_div.classList.add('pc_promo_div')
                ec_promo_div.classList.add('empty');
                var view = elproductPriceBlock
                ...
                if (document.querySelector('.pc_promo_div') != null){
                    if (productBlock.querySelector('.pc_promo_div') != null){
                        productBlock.querySelector('.pc_promo_div').innerText = ""
                        productBlock.querySelector('.pc_promo_div').classList.add("empty");
                    }
                }
                // if (document.querySelector('.inc_offer') != null){
                //     productBlock.querySelector('.inc_offer').remove()
                // }
            }
        }
...
function clickListener(e) {
    ...
    if (clickedElement != null) {
        ...
        try {
            ...
            if (clickedElement.classList.contains("inc_product_desc_price_block") || clickedElement.classList.contains("inc_product_variant_simple_text") || clickedElement.classList.contains("pc_promo_div") || clickedElement.classList.contains("inc_avail_offer_text") || clickedElement.classList.contains("pc_promo_span") || clickedElement.classList.contains("inc_promo_span_close_parent")) {
                if(document.querySelector('.inc_promo_span_close_parent') != null){
                    document.querySelector('.inc_promo_span_close_parent').click();
                } 
            }
        ...
        if (sbBlock != null) {
            ...
            var img_elempdp = setInterval(function () {
                ...
                if (circle_img != null) {
                    ...
                    var prdname = ""
                    if (document.querySelector('.pdp-heading-ratings__title') != null) {
                        prdname = document.querySelector('.pdp-heading-ratings__title').textContent
                    }
                    ...
                    if (prd_prc != null) {
                        ...
                        if (prd_prc != null) {
                            if (subtotalinc_active != null) {
                                ...
                                document.querySelector('.inc_cart_added_product_desc_title_text').innerText = qty + "X" + prdname
