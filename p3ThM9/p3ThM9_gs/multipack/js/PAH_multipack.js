...
INC_COL.config.multipackof = ""
INC_COL.config.price_pack_for=""
...
setTimeout(function(){
    ...
    var jsFilePath1 = "https://www.increasingly.co/Implementation/p3ThM9/p3ThM9_gs/multipack/js/p3ThM9_multi_org.js?v=" + versionUpdate1;
                    ...
                    for (var i = 0; i < bundles.Products.length; i++) {
                        ...
                        if(INC_COL.config.uniqueobjectproduct.indexOf(irbProductsArray.ProductId) == -1){
	                        ...
	                        if(INC_COL.config.pdpPID == product.ProductId){
                              ...
                              if((product.Field4.indexOf("Online Only") >= 0 || product.Field4.indexOf("for") >= 0) && (bundleResponse.Collections[c].ComboId == "876856" || bundleResponse.Collections[c].ComboId == "876857")){
                                if(product.Field4.split('-')[1] != undefined){
                                    INC_COL.config.multipackof =  product.Field4.split('-')[1].split('for')[1].trim()
                                }else if(product.Field4.indexOf("for") >= 0){
                                    if(product.Field4.split('-')[1] == undefined && product.Field4.split('for')[1] != undefined){
                                        if(product.Field4.split('for')[1].indexOf('£') >= 0){
                                            INC_COL.config.price_pack_for = product.Field4.split('for')[1].replace('£','').trim()
                                            INC_COL.config.multipackof = 1 //product.Field4.split('for')[0].trim()
                                        }
                                    }
                                }
                              }
...
INC_COL.methods.createcolProductShowcaseBlock = function (pdpProductId, blockType, bundleProductListCheck, parentblock,elProductShowcaseBlock,mainprod) {
    ...
    if(productObj.PromotionalMessage != 'null' && productObj.PromotionalMessage != "" && productObj.PromotionalMessage != undefined){
        if(INC_COL.config.multipackof == "" && INC_COL.config.price_pack_for == ""){
    ...
    if(INC_COL.config.multipackof != null && INC_COL.config.multipackof != "" && INC_COL.config.multipackof != undefined){
        if(INC_COL.config.price_pack_for != "" &&  INC_COL.config.price_pack_for != null){
            var activePrice = parseFloat(INC_COL.config.price_pack_for * INC_COL.config.multipackof).toFixed(2).toString();
            var regularPrice = parseFloat(INC_COL.config.price_pack_for * INC_COL.config.multipackof).toFixed(2).toString();
            var totalsaveprc = get_discount_price(INC_COL.config.price_pack_for * INC_COL.config.multipackof, INC_COL.config.price_pack_for * INC_COL.config.multipackof)
            var totalsaveper = get_discount_price_percentage(INC_COL.config.price_pack_for * INC_COL.config.multipackof, INC_COL.config.price_pack_for * INC_COL.config.multipackof)
        }else{
            var activePrice = parseFloat(productObj.activePriceMain * INC_COL.config.multipackof).toFixed(2).toString();
            var regularPrice = parseFloat(productObj.regularPriceMain * INC_COL.config.multipackof).toFixed(2).toString();
            var totalsaveprc = get_discount_price(productObj.regularPriceMain * INC_COL.config.multipackof, productObj.activePriceMain * INC_COL.config.multipackof)
            var totalsaveper = get_discount_price_percentage(productObj.regularPriceMain * INC_COL.config.multipackof, productObj.activePriceMain * INC_COL.config.multipackof)
        }
...
INC_COL.methods.createcolProductModalBlock = function (pdpProductId, blockType, bundleProductListCheck, parentblock,elmodalblock,mainprod) {
    ...
    if(INC_COL.config.multipackof != null && INC_COL.config.multipackof != "" && INC_COL.config.multipackof != undefined){
        if(INC_COL.config.price_pack_for != "" &&  INC_COL.config.price_pack_for != null){
            var totalsaveprc = get_discount_price(INC_COL.config.price_pack_for *  INC_COL.config.multipackof, productObj.activePriceMain *  INC_COL.config.multipackof)
            var activePrice = parseFloat(INC_COL.config.price_pack_for *  INC_COL.config.multipackof).toFixed(2).toString();
            var regularPrice = parseFloat(INC_COL.config.price_pack_for *  INC_COL.config.multipackof).toFixed(2).toString();
            var totalsaveper =  get_discount_price_percentage(INC_COL.config.price_pack_for *  INC_COL.config.multipackof, INC_COL.config.price_pack_for *  INC_COL.config.multipackof)
        }else{
    ...
    if (Object.keys(productObj).length != 0) {
        ...
        if(INC_COL.config.multipackof != null && INC_COL.config.multipackof != "" && INC_COL.config.multipackof != undefined){
            if(INC_COL.config.price_pack_for != "" &&  INC_COL.config.price_pack_for != null){
                ...
                var totalsaveprc = get_discount_ecirp(INC_COL.config.price_pack_for * INC_COL.config.multipackof, INC_COL.config.price_pack_for * INC_COL.config.multipackof)
                var totalsaveper = get_discount_ecirp_percentage(INC_COL.config.price_pack_for * INC_COL.config.multipackof, INC_COL.config.price_pack_for * INC_COL.config.multipackof)
            }else{
                var activePrice = parseFloat(productObj.activePrice * INC_COL.config.multipackof).toFixed(2).toString();
                var regularPrice = parseFloat(productObj.regularPrice * INC_COL.config.multipackof).toFixed(2).toString();
                
                var totalsaveprc = get_discount_ecirp(productObj.regularPrice * INC_COL.config.multipackof, productObj.activePrice * INC_COL.config.multipackof)
                var totalsaveper = get_discount_ecirp_percentage(productObj.regularPrice * INC_COL.config.multipackof, productObj.activePrice * INC_COL.config.multipackof)
            }
        }else{
...
INC_COL.methods.calculateBundleTotalcol = function () {
                ...
                for (childProductId in productListObj[mainProductId]) {
                    if (sku == productListObj[mainProductId][childProductId]["sku"]) {
                        if (duplicate_check_produ.indexOf(productListObj[mainProductId][childProductId]["sku"]) == -1) {
                            ...
                            if(INC_COL.config.price_pack_for != "" &&  INC_COL.config.price_pack_for != null){
                                var regularPrice = INC_COL.config.price_pack_for;
                                var activePrice = INC_COL.config.price_pack_for;
                            }else{
    ...
    if(INC_COL.config.multipackof != null && INC_COL.config.multipackof != "" && INC_COL.config.multipackof != undefined){
        var priceObj = {
            totalActivePrice: parseFloat(totalActivePrice *  INC_COL.config.multipackof).toFixed(2),
            totalRegularPrice: parseFloat(totalRegularPrice *  INC_COL.config.multipackof).toFixed(2),
