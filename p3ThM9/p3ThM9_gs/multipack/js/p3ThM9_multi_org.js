...
INC.methods.getProductIdFromWebPage = function () {
    ...
    if((window.location.href.indexOf('incgs=1') >= 0 || window.location.href.indexOf('incgs=2') >= 0 || window.location.href.indexOf('incgs=3') >= 0) && cid != "" && cid != 0 && cid != null && pid != "" && pid != 0 && pid != null){
        var total_split_href=href_split.split('&');
        var cid="";
        var pid=""
    ...
    if (sbBlock != null) {
        ...
        bundleCartProductsArray.forEach(function (productSKU) {
            ...
            if (INC.clientConfig.OOS.indexOf(productObj.mainId) >= 0) {
                ...
                var elImgTag = INC.methods.generateHTMLTags('img', { src: productObj.combo });
                if(onloadpdp == true){
                    elImgTag.src = productObj.imageURL
                }
                elImgTag.setAttribute('onerror', "this.src='" + INC.config.noimage + "'");
        ...
        if (+subtotalactiveprice != null && (+subtotalactiveprice < +subtotalregukarprice)) {
            ...
            elHeadersubtotalactivetext.style.color = "#000"
            elHeadersubtotalregulartext.setAttribute('subtotalRegularprice', subtotalregukarprice)
            elHeadersubtotalactivetext.innerHTML = "<span>" + "was" + "</span>" + INC.clientConfig.currencySymbol + formatter.format(subtotalregukarprice)
            elHeadersubtotalregulartext.innerHTML = ""
        }
...
INC.methods.updateSidebarColBlock = function (COL_dataStore) {
    ...
    if(sbBlock != null){
        ...
        if (Object.keys(dataStore.bundleCartProducts).length == 1) {
            ...
            sidebarHeaderItemCountTitleText.innerHTML = "Subtotal " + "(" +  Object.keys(dataStore.bundleCartProducts).length + " items)"
        }
        var headerItemCountFigureText = 0;
        var subtotalactiveprice = 0
        var subtotalregukarprice = 0
        ...
        bundleCartProductsArray.forEach(function (productSKU, i) {
            ...
            var elAttColorText = productBlock.querySelector(getSelectorClassName(classNames, "sidebarCartAddedProductDescAttColorText"));
            var elAttsizeText = productBlock.querySelector(getSelectorClassName(classNames, "sidebarCartAddedProductDescAttSizeText"));
            var elImgTag = INC.methods.generateHTMLTags('img', { src: productObj.comboimg });
            ...
            elImgTag.setAttribute('onerror', "this.src='" + INC.config.noimage + "'");
            elImg.appendChild(elImgTag);
            elImgTag.setAttribute('alt', productObj.ComboTitle)
            elTitleText.innerHTML = dataStore.bundleCartProducts[productSKU] + "<small>x</small>" + productObj.combotitle;
            if (color != "") {
                if (size != "") {
            ...
            if(INC_COL.config.multipackof != null && INC_COL.config.multipackof != "" && INC_COL.config.multipackof != undefined){
                if(INC_COL.config.price_pack_for != "" &&  INC_COL.config.price_pack_for != null){
                    var activePrice = parseFloat(INC_COL.config.price_pack_for * INC_COL.config.multipackof).toFixed(2)
                    var regularPrice = parseFloat(INC_COL.config.price_pack_for * INC_COL.config.multipackof).toFixed(2)
                }else{
                    var activePrice = parseFloat(productObj.activePrice).toFixed(2) * INC_COL.config.multipackof;
                    var regularPrice = parseFloat(productObj.regularPrice).toFixed(2) * INC_COL.config.multipackof;
                }
                
            }else{
                var activePrice = parseFloat(productObj.activePrice).toFixed(2) * INC_COL.config.collectioncount;
                var regularPrice = parseFloat(productObj.regularPrice).toFixed(2) * INC_COL.config.collectioncount;
            }
            subtotalactiveprice = (+subtotalactiveprice + (+activePrice * dataStore.bundleCartProducts[productSKU]));
            subtotalregukarprice = (+subtotalregukarprice + (+regularPrice * dataStore.bundleCartProducts[productSKU]));
        ...
        // elHeadersubtotalsavetext.innerHTML = "SAVE " + formatter.format(discountPrice) + " (" + (discountPer) + "%)";
            ...
            elHeadersubtotalregulartext.innerHTML = "<span>" + "was" + "</span>" + INC.clientConfig.currencySymbol + formatter.format(subtotalregukarprice).replace('US', '')
...
INC.methods.createProductShowcaseBlock = function (pdpProductId, blockType, bundleProductListCheck, parentblock) {
    ...
    if(productObj.ProductType != "simple"){
        ...
        if(priceRange.innerText.split(',')[0] != priceRange.innerText.split(',')[1] && priceRange.innerText.split(',')[1] != undefined){
            ...
            elPriceRegularText.innerHTML = "<span>" + "WAS" + "</span>" + priceRange.innerText.split(',')[1]
        }else{
            elPriceActiveText.innerHTML = priceRange.innerText.split(',')[0]
        }
    }else{
