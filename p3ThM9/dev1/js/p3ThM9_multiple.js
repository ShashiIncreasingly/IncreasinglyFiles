...
INC.dataStore.methods = function () {
    ...
    function buildDataStoreObj(bundleResponse) {
        ...
        if (window.location.pathname == "/shop/Basket" || INC.config.recommendation) {
            var productIds_d = [];
            if (window.oiProducts != undefined) {
                Array.prototype.forEach.call(window.oiProducts, function (elProductCode) {
                    var productId = elProductCode.id;
...
//    this.url = ((product != null) && (product != undefined)) ? product["childProductUrl"] : "";
   if(window.location.host == "www.uat03p.pahtest.com"){
    ...
    if (product["ProductLevelDiscountMessage"] != null) {
       ...
       // this.ProductLevelDiscountMessage ="Buy One Get One Half Price"
    }
    this.pointcollect = "";
    this.manufacturer = ((product != null) && (product != undefined)) ? product["Manufacturer"] : "";
...
INC.methods.refreshBundles = function (nwConfigObj) {
    ...
    INC.methods.networkReq(nwConfigObj, function (response) {
        ...
        if (responseJSON.hasOwnProperty('Bundles') && (responseJSON.Bundles != null)) {
            ...
            switch (INC.config.pageType) {
                case "pdp": INC.methods.createAMBlock("PDP"); break;
            }
        }
    });
}
    ...
    if (INC.dataStore.dataStoreObj.undefined == undefined) {
        ...
        Array.prototype.forEach.call(amProductBlocks, function (productBlock) {
            productBlock.querySelector('.inc_product_desc_add_block').addEventListener("click", function (e) {
                ...
                var add_btn_click = productBlockNode.querySelector('.inc_product_desc_add_text');
...
INC.methods.createPDPBlock = function (sidebar) {
    ...
    if (sidebar != "sidebar") {
        if (INC.dataStore.dataStoreObj.undefined != undefined || empty_sidebarflag == false) {
            ...
            var inc_bundle_list_block = generate_html_tag('div','inc_bundle_list_block')
            for(var b=0;b<INC.config.bundledid.length;b++){
                var inc_bundle_list_items_bundle_block = generate_html_tag('div','inc_bundle_list_items_bundle_block')
                var inc_bundle_list_items_block = generate_html_tag('div','inc_bundle_list_items_block')
                inc_bundle_list_items_bundle_block.appendChild(inc_bundle_list_items_block)
                inc_bundle_list_block.appendChild(inc_bundle_list_items_bundle_block)
                for(var p=0;p<INC.config.bundledid[b].productids.length;p++){
                    if(p != 0){
                        var inc_plusicon = generate_html_tag('span','inc_plusicon')
                        inc_bundle_list_items_block.appendChild(inc_plusicon)
                    }
                    var mainProductBlock = this.createProductModuleBlock(INC.config.bundledid[b].productids[p], "pdp", false, pdpBlock);
                    inc_bundle_list_items_block.appendChild(mainProductBlock)
                    if( p == 1){
                        // var pdpBundleCartBlock = this.createPDPBundleCartBlock(pdpBlock);
                        // inc_bundle_list_items_checkout_block.appendChild(pdpBundleCartBlock)
                    }
                }
                var cartsummary = generate_html_tag('div','inc_pdp_bundle_cart_summary_block')
                var cartsumhtml = '<div class="inc_pdp_bundle_cart_summary_title_block"><div class="inc_pdp_bundle_cart_summary_title_text_block"><div class="inc_pdp_bundle_cart_summary_title_text">Total bundle price</div></div></div><div class="inc_pdp_bundle_cart_summary_price_block simple_price"><div class="inc_pdp_bundle_cart_summary_price_active_block"><div class="inc_pdp_bundle_cart_summary_price_active_text_block"><div class="inc_pdp_bundle_cart_summary_price_active_text">£00.00</div></div></div><div class="inc_pdp_bundle_cart_summary_price_regular_block"><div class="inc_pdp_bundle_cart_summary_price_regular_text_block"><div class="inc_pdp_bundle_cart_summary_price_regular_text" style="display: none;"></div></div></div></div><div class="inc_pdp_bundle_cart_summary_add_block"><div class="inc_pdp_bundle_cart_summary_add_btn_block" style="pointer-events: auto;opacity: 1;"><div class="inc_pdp_bundle_cart_summary_add_btn_text_block"><div class="inc_pdp_bundle_cart_summary_add_btn_text" style="text-transform: none;">Add bundle to basket</div><div class="inc_pdp_bundle_cart_summary_add_btn_span"></div></div><div class="inc_pdp_bundle_cart_summary_add_btn_img_block"><div class="inc_pdp_bundle_cart_summary_add_btn_img"></div></div></div></div>'
                cartsummary.innerHTML = cartsumhtml
                inc_bundle_list_items_checkout_block.appendChild(cartsummary)
            }
            pdpBlock.querySelector('.inc_pdp_bundle_block').appendChild(inc_bundle_list_block)
...
INC.methods.updateSidebarBlock = function () {
    ...
    if (sbBlock != null) {
        ...
        var elHeaderItemCountFigureText = sbBlock.querySelector(".inc_header_item_count_figure_text");
        var elHeadersubtotalactivetext = sbBlock.querySelector(".inc_cart_added_product_desc_subtotal_price_active_text");
        var elHeadersubtotalregulartext = sbBlock.querySelector(".inc_cart_added_product_desc_subtotal_price_regular_text");
        ...
        bundleCartProductsArray.forEach(function (productSKU) {
            ...
            if (INC.clientConfig.OOS.indexOf(productObj.mainId) >= 0) {
                ...
                var addedhtml = '<div class="inc_cart_added_product_img_block"> <div class="inc_cart_added_product_img"><img class="inc_lazy" src="'+productObj.imageURL+'" data-src="'+productObj.imageURL+'" onerror="this.src='+INC.config.noimage+'" alt="'+productObj.name+'"></div></div><div class="inc_cart_added_product_desc_block"> <div class="inc_cart_added_product_desc_title_block"> <div class="inc_cart_added_product_desc_title_text_block"> <div class="inc_cart_added_product_desc_title_text" title="Easidri Cooling Coat Wide Fit" style="text-transform: unset;">'+dataStore.bundleCartProducts[productSKU] + ' X ' + productObj.name+'</div></div></div><div class="inc_cart_added_product_desc_att_block"> <div class="inc_cart_added_product_desc_att_color_block"> <div class="inc_cart_added_product_desc_att_color_text_block"> <div class="inc_cart_added_product_desc_att_color_text_block"></div></div></div><div class="inc_cart_added_product_desc_att_size_block"> <div class="inc_cart_added_product_desc_att_size_text_block"> <div class="inc_cart_added_product_desc_att_size_text">'+sel_size+'</div></div></div></div><div class="inc_cart_added_product_desc_price_block"> <div class="inc_cart_added_product_desc_price_active_block"> <div class="inc_cart_added_product_desc_price_active_text_block"> <div class="inc_cart_added_product_desc_price_active_text">'+formatter.format(activePrice)+'</div><div class="inc_cart_added_product_desc_price_active_text_msg"></div></div></div><div class="inc_cart_added_product_desc_price_regular_block"> <div class="inc_cart_added_product_desc_price_regular_text_block"> <div class="inc_cart_added_product_desc_price_regular_text"></div></div></div></div><div class="inc_cart_added_product_qty_block"></div></div><div class="sidebar_product_quantity_label">'+dataStore.bundleCartProducts[productSKU]+'</div>'
        ...
        if (+subtotalactiveprice != null && (+subtotalactiveprice < +subtotalregukarprice)) {
            ...
            elHeadersubtotalregulartext.innerHTML = "<span>" + "Was" + "</span>" + formatter.format(subtotalregukarprice)
                    ...
                    if (INC.config.sidebarremovedprod.indexOf(firstProductObjByMainId.id) == -1) {
                        if (dupl_sidebar_prod.indexOf(firstProductObjByMainId.id) == -1) {
                            ...
                            productBlock.querySelector('.inc_product_desc_add_block').addEventListener("click", function (e) {
                                ...
                                var qty = productBlockNode.querySelector('input').value;
                                var add_btn_click = productBlock.querySelector('.inc_product_desc_add_text')
                                var add_prod_im = productBlock.querySelector('.inc_product_desc_add_img')
                                ...
                                if (added_maxqtyel == null) {
                                    setTimeout(function () {
                                        Array.prototype.forEach.call(productBlockNode.querySelectorAll('.inc_product_desc_add_img'), function (el) {
                                    ...
                                    if (productBlockNode.querySelector('.img_color_swatch') != null) {
                                        if (productBlockNode.querySelector('.inc_product_modal_block').querySelector('.inc_product_desc_att_color_list_item_block.inc_active') != null) {
                                            if (INC.config.disablebtn == true) {
                                                INC.config.disablebtn = false;
                                    ...
                                    } else if (productBlockNode.querySelector('.inc_product_modal_block').querySelector('.inc_product_desc_att_size_list_item_img_block.avail') != null) {
                                        if (productBlockNode.querySelector('.inc_product_modal_block').querySelector('.inc_product_desc_att_size_list_item_img_block.avail.inc_active') != null) {
                                            if (INC.config.disablebtn == true) {
                                                INC.config.disablebtn = false;
                                    ...
                                    INC.config.disablebtn = true
                                    jQuery('.inc_sidebar_modal_block.inc_active').animate({ scrollTop: jQuery('.inc_sidebar_cart_added_block.inc_active').height() }, 'fast');
                                }
                            });
...
INC.methods.addProductToSidebarCart = function (findObj, qty, productBlock, client_add_btn, btn_addtocart) {
    ...
    var bundleaddedpriceblock = sbBlock.querySelector(".inc_cart_added_product_desc_subtotal_price_block");
    var elHeadersubtotalactivetext = sbBlock.querySelector(".inc_cart_added_product_desc_subtotal_price_active_text");
    var elHeadersubtotalregulartext = sbBlock.querySelector(".inc_cart_added_product_desc_subtotal_price_regular_text");
    var elHeaderItemCountFigureText = sbBlock.querySelector(".inc_header_item_count_figure_text");
    var sidebarHeaderItemCountTitleText = sbBlock.querySelector(".inc_header_item_count_title_text");
...
INC.methods.createProductModuleBlock = function (pdpProductId, blockType, bundleProductListCheck, parentblock) {
    if (INC.dataStore.dataStoreObj.undefined == undefined) {
        ...
        var productShowcaseBlock = INC.methods.createProductShowcaseBlock(pdpProductId, blockType, bundleProductListCheck, parentblock,elProductShowcaseBlock);
        var productModalBlock = INC.methods.createProductModalBlock(pdpProductId, blockType, bundleProductListCheck, parentblock,elProductModalBlock);
...
INC.methods.createProductShowcaseBlock = function (pdpProductId, blockType, bundleProductListCheck, parentblock,elProductShowcaseBlock) {
    ...
    var elproductPriceBlock = productBlock.querySelector('.inc_product_desc_price_block');
    ...
    var elQtyBlock = productBlock.querySelector('.inc_product_desc_qty_block');
    var elQtyInputText = productBlock.querySelector('.inc_product_desc_qty_input_text');
    var elQtyDownBlock = productBlock.querySelector('.inc_product_desc_qty_down_block');
    var elQtyUpBlock = productBlock.querySelector('.inc_product_desc_qty_up_block');
    ...
    var productDescQtyUp = productBlock.querySelector('.inc_product_qty_up_block');
    var productDescQtyDown = productBlock.querySelector('.inc_product_qty_down_block');
    ...
    if ((productObj.otherImageList != null) && productObj.otherImageList.length > 1) {
        ...
        productObj.otherImageList.forEach(function (otherImgURL) {
            ...
            var imglist = '<div class="inc_product_img_gallery_list_item_img_block"> <div class="inc_product_img_gallery_list_item_img"><img class="inc_lazy" data-src="'+otherImgURL+'" src="'+otherImgURL+'" role="img" aria-label="Image" onerror="this.src='+INC.config.noimage+'" alt="'+productObj.name+'"></div></div><div class="inc_product_img_gallery_list_item_text_block"> <div class="inc_product_img_gallery_list_item_text"></div></div>'
    ...
    if(productObj.ProductType != "simple"){
        ...
        if (activePrice != regularPrice && regularPrice != "" && regularPrice != null && regularPrice != 0) {
            ...
            if (productObj.pricingCount != "") {
                ...
                elPriceRegularText.innerHTML = "<span>" + "WAS" + "</span>" + INC.clientConfig.regularPricePrefix + formatter.format(regularPrice)
            }
            else {
                elPriceActiveText.innerHTML = formatter.format(activePrice)
                elPriceRegularText.innerHTML = "<span>" + "WAS" + "</span>" + INC.clientConfig.regularPricePrefix + formatter.format(regularPrice)
    ...
    elselectoptaddbtn.addEventListener('click', function () {
        ...
        if (this.querySelector('.inc_product_variant_simple_text.simple') != null) {
            ...
            //if (document.querySelector('#shopify-section-cart--slideout .cd-panel--is-visible') != null) {
...
INC.methods.createProductModalBlock = function (pdpProductId, blockType, bundleProductListCheck, parentblock,elmodalblock) {
    ...
    // productBlock.querySelector('.inc_product_header_block').innerHTML = ""
    findObj = new INC.classes.FindObj({ mainId: productObj.mainId });
        ...
        if (type_of_variant == "zero" || type_of_variant == "size" || type_of_variant == "third") {
            ...
            if (productColorsObj["colorArray"].length > 0) {
                ...
                productColorsObj["colorArray"].forEach(function (colorObj) {
                    ...
                    var colorSwatchImg = generate_html_tag('img');
                    colorSwatchImg.src = colorObj["imgSrc"]
...
INC.methods.onloadCheckproduct = function (productModuleBlock) {
    ...
    modalProductBlock.querySelector('.inc_product_desc_add_img').classList.add("checked");
    if (INC.config.pageType == "pdp") INC.methods.updateBundleCart({ sku: sku, qty: qty }, "add", main_id, bundle_id, pname, pprice);
...
INC.methods.createProductListItemBlock = function (productObj, blockType) {
    var productListItemBlock = generate_html_tag('inc_pdp_bundle_product_list_item_block');
...
INC.methods.updateBundleCartBlock = function (priceObj) {
    ...
    if (elPdpBundleCartBlock != null) {
        ...
        if (totalRegularPrice != "0.00" && totalRegularPrice != totalActivePrice && totalRegularPrice != null && totalRegularPrice != "") {
            elBundleCartPriceRegularText.innerHTML = "<span>" + "Was" + "</span>" + formatter.format(totalRegularPrice)
        ...
        var elPriceRegularText = elPdpBundleCartBlock.querySelector('.inc_pdp_bundle_cart_summary_price_regular_text');
...
INC.methods.createBundleCartAddedProductBlock = function (productId, qty, blockType) {
    ...
    elEditTextBlock.addEventListener("click", function () {
        if (INC.config.deviceType == "mobile") {
            if (INC.debug) console.log(productObj);
            if (INC.debug) console.log(document.querySelector('.inc_pdp_bundle_product_list_item_block' + '[data-main_id="' + productObj.mainId + '"]'));
        ...
        if (INC.config.deviceType == "desktop") {
            if (INC.debug) console.log(document.querySelector('.inc_product_module_block' + '[data-id="' + productObj.id + '"]'));
            Array.prototype.forEach.call(document.querySelector(getSelectorClassName(classNames, "pdpBundleProductListMainBlock")).querySelectorAll('.inc_product_module_block'), function (el, index) {
    ...
    dojo.xhrPost({
        ...
        load: function(_255, _256) {
            ...
            if(_255.errorMessage != undefined && _255.errorMessage != ""){
                ...
                if(document.querySelector('.inc_justerror') == null){
                    ...
                    errormes.appendChild(errormestext)
                    headertitle.parentNode.insertBefore(errormes,headertitle)
                    jQuery('.inc_sidebar_modal_block.inc_active').animate({ scrollTop: jQuery('.inc_header_title_text_block').height() - 100 }, 'fast');
                    setTimeout(function(){
                        var remerrorjust=document.querySelectorAll('.inc_justerror')
                        for(r=0;r<remerrorjust.length;r++){
                            if(remerrorjust[r] != null){
        ...
        error: function(_25c, _25d) {
            ...
            if (INC.config.pageType == "cartPage" || INC.config.recommendation) {
                ...
                productBlock.querySelector('.inc_product_desc_add_text').innerText = "Not Added";
        ...
        try {
            ...
            if(INC.config.pageType == "pdp"){
                if(clickedElement.id == "add_to_cart" && !(clickedElement.classList.contains('btn--repeat')) && clickedElement.parentNode.className != "click-collect__button"){
                    ...
                    if(qty_check >=1 && client_var_check){
                        pdp_listeners();
                    } 
                }       
            }
            if(clickedElement.classList.contains('basket-product__remove-item') == true){
                setTimeout(function(){
                    INC.init();
                },2000)
            }
...
function inc_showSlides(n, ind, elm,type_of,arrow) {
    ...
    if(arrow == "arrow"){
        if(modal_dots.length > 5 && window.innerWidth > 819){
