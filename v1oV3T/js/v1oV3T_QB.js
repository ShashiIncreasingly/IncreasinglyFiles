...
(function () {
    ...
    function get_html_element(element, classnames) {
        var generateEl = document.createElement(element);
        if (classnames != undefined) {
            generateEl.classList.add(...classnames);
        }
        return generateEl;
    }

    function prepareqty_bundles(qty_bundles, qty, orignal) {
        let seperate = qty_bundles
        let output = null
        for (let i = 0; i < seperate.length; i++) {
            let individual = seperate[i]
            let thisQty = individual[1]
            if (Number(qty) >= Number(thisQty)) {
                output = {
                    activePrice: individual[0],
                    quantity: thisQty
                }
            }
        }
                                            ...
                                            if (Object.prototype.hasOwnProperty.call(pAttributes, pattv)) {
                                                ...
                                                for (let ptr in pAttributesValues) {
                                                    if (Object.prototype.hasOwnProperty.call(pAttributesValues, ptr)) {
                                                        ...
                                                        childProduct["QuantityBasedBundle"] = product["QuantityBasedBundle"]
    ...
    INC.classes.ProductObj = function (product) {
        ...
        //this.qty_bundles = product["Field3"] != null ? product["Field3"] : null;
        this.qty_bundles = "£1.15,1|£1.1,5|£1.05,10|£1.03,25|£1.0,50"
    ...
    INC.classes.ProductAttObj = function (product) {
        ...
        //this.qty_bundles = product["childProductField3"] != null ? product["childProductField3"] : null;
        this.qty_bundles = "£1.15,1|£1.1,5|£1.05,10|£1.03,25|£1.0,50"
        this.QuantityBasedBundle = ((product != null) && (product != undefined)) ? product["QuantityBasedBundle"] : [];
        this.size = "";
        this.option = {};
    ...
    function prepareqty(productBlock, blockType) {
        ...
        function qtyupdateInc(_elm, prdblk, eladtext) {
            ...
            // QTY BUNDLING
            handleManualQtyBundling(productBlock, qty)
        }
    ...
    function handleManualQtyBundling(productBlock, qty) {
        if (productBlock.querySelector('.inc_qtybundle_options_block')) {
            let bundles = productBlock.querySelector('.inc_qtybundle_options').getAttribute('bundles')


            if (productBlock.querySelector('.inc_qtybundle_options_block[qty="' + qty + '"]')) {
                productBlock.querySelector('.inc_qtybundle_options_block[qty="' + qty + '"]').click()
            } else {
                //modal block
                if (productBlock.parentNode.parentNode.querySelector('.inc_product_modal_block .inc_qtybundle_options_block.inc_qtybundle_options.inc_active')) {
                    productBlock.parentNode.parentNode.querySelector('.inc_product_modal_block .inc_qtybundle_options_block.inc_qtybundle_options.inc_active').classList.remove('inc_active')
                }
                //showcase block
                if (productBlock.parentNode.parentNode.querySelector('.inc_product_showcase_block .inc_qtybundle_options_block.inc_qtybundle_options.inc_active')) {
                    productBlock.parentNode.parentNode.querySelector('.inc_product_showcase_block .inc_qtybundle_options_block.inc_qtybundle_options.inc_active').classList.remove('inc_active')
                }

                if (productBlock.querySelector('.inc_qtybundle_options_block.inc_qtybundle_options.inc_active')) {
                    productBlock.querySelector('.inc_qtybundle_options_block.inc_qtybundle_options.inc_active').classList.remove('inc_active')
                }
                if (productBlock.querySelector('.inc_qtybundle_select')) {
                    productBlock.querySelector('.inc_qtybundle_select').innerText = "Quantity " + qty
                } else {
                    if (productBlock.parentElement.parentElement.querySelector('.inc_qtybundle_select')) {

                        productBlock.parentElement.parentElement.querySelector('.inc_qtybundle_select').innerText = "Quantity " + qty
                    }
                }
            }
        }
    }
    function create_qty_bundling_block(qty_bundles, productBlock, sourceBlock, price_sibling, blockType, productObj) {

        // Check if simpel & bundle
        if (productBlock.classList.contains('simple')) {
            productBlock.classList.add('qty_with_simple')
        }
        // Bundles Array
        let bundles_array = qty_bundles
        let parent = get_html_element('div', ['inc_qtybundle'])
        if (blockType == 'showcase') {
            parent = get_html_element('div', ['inc_qtybundle', 'inc_hide'])
        } else if (blockType == 'modal') {
            parent = get_html_element('div', ['inc_qtybundle', 'inc_qtybundle_modal'])
        } else {
            parent = get_html_element('div', ['inc_qtybundle'])
        }

        // For Client Side
        let qty_bundling_block = get_html_element('div', ['inc_product_qty_bundling_block', 'inc_client_bundling'])
        // Loop Over Array
        // Create Main Loop Parent
        let select = get_html_element('div', ['inc_qtybundle_options', 'inc_qty_bundle_select_child']) // Select
        let select_title = get_html_element('div', ['inc_qtybundle_options_title', 'inc_qty_bundle_select_title']) // Select Title
        select_title.innerText = "Select a Pack:" // Add Title For Select
        // Add Title as a child of select
        let select_title_client = get_html_element('div', ['inc_qtybundle_options_title_client', 'inc_qty_bundle_select_title_client'])
        select_title_client.innerHTML = "<span>Please Select</span>" // Add Title For Select
        let inc_qty_bundle_selected_parent = get_html_element('div', ['inc_qty_bundle_selected_parent'])
        //inc_qty_bundle_selected_parent.classList.add("inc_active")
        select.appendChild(select_title)
        if (blockType == "client") {
            inc_qty_bundle_selected_parent.appendChild(select_title_client)
            select.appendChild(inc_qty_bundle_selected_parent)
            let inc_product_qty_bundling_last_element = get_html_element('div', ['inc_product_qty_bundling_last_element'])
            inc_product_qty_bundling_last_element.innerHTML = "or"
            select.appendChild(inc_product_qty_bundling_last_element)
            select_title_client.addEventListener("click", function (e) {
                inc_qty_bundle_selected_parent.classList.toggle("inc_active")
            })
        }
        select.setAttribute('bundles', bundles_array)
        select.setAttribute('active_qty', null)

        // Inside Options
        let select_option_parent = get_html_element('div', ['inc_qtybundle_options_block_parent', 'inc_qtybundle_options_individual'])

        // Loop Over Bundles

        bundles_array.map(function (bundle) {

            const [price, qty] = [Number(bundle) * productObj.activePrice, Number(bundle)]
            let options = get_html_element('div', ['inc_qtybundle_options_block', 'inc_qtybundle_options'])
            if (blockType == 'showcase') {
                if (window.innerWidth < 820) {
                    options.innerHTML = `<div><span>Pack of ${qty}</span><span>${price}</span></div>`
                } else {
                    options.innerHTML = `<div><span>Pack of ${qty}</span><span>${price}</span></div>`
                }
            } else if (blockType == "modal") {
                options.innerHTML = `<div><span>Pack ${qty}</span> - <span>${price}</span></div>`
            } else {
                options.innerHTML = `<div><span>Pack of ${qty}</span><span>${price}</span></div>`
                if (blockType == "client") {
                    if (document.querySelector('.product_quantity_wrapper input') != null) {
                        let qty11 = document.querySelector('.product_quantity_wrapper input').value
                        if (qty == qty11) {
                            setTimeout(function () {
                                options.click()
                            }, 100)
                            //options.classList.add("inc_active3");
                        }
                    }
                }
            }

            select_option_parent.appendChild(options)
            options.setAttribute('qty', qty)
            // Event Listener
            options.addEventListener('click', function () {
                // closeModal(parent) // Hide Modal
                this.classList.add('inc_active') // Make Current Active
                // Remove Other

                let current = this

                this.parentElement.querySelectorAll('.inc_qtybundle_options_block').forEach(function (ele) {
                    if (ele !== current) {
                        ele.classList.remove('inc_active')
                    }
                })
                select.setAttribute('active_qty', qty)
                // Call Update
                let otherBlock = null


                if (blockType == "showcase") {
                    update_prices_for_qty_bundling(productBlock, qty, price, "showcase", false)
                    otherBlock = productBlock.parentElement.parentElement.querySelector('.inc_product_modal_block')
                    update_prices_for_qty_bundling(otherBlock, qty, price, "modal", false)
                }
                if (blockType == "modal") {
                    update_prices_for_qty_bundling(productBlock, qty, price, "modal", false)
                    otherBlock = productBlock.parentElement.parentElement.querySelector('.inc_product_showcase_block')
                    update_prices_for_qty_bundling(otherBlock, qty, price, "showcase", false)
                }
                if (blockType == "client") {
                    if (window.innerWidth < 820) {
                        document.querySelector('.inc_qtybundle_options_title_client.inc_qty_bundle_select_title_client').innerHTML = "<span class='inc_main_title_selected_text'>Pack of " + qty + "</span>"
                    } else {
                        document.querySelector('.inc_qtybundle_options_title_client.inc_qty_bundle_select_title_client').innerHTML = "<span class='inc_main_title_selected_text'>Pack of " + qty + " - " + "<span>" + price + "</span></span>"
                    }
                    //document.querySelector('.inc_qtybundle_options_title.inc_qty_bundle_select_title').innerHTML = "Select a Pack: Pack of " + qty + " (" + price + " each)"
                    otherBlock = productBlock.parentNode.parentNode.querySelector('.inc_product_showcase_block')
                    update_prices_for_qty_bundling(otherBlock, qty, price, "showcase", false)

                    // Update Client Input	
                    var event = new Event('keyup');
                    if (document.querySelector('.product_quantity_wrapper input')) {
                        document.querySelector('.product_quantity_wrapper input').value = qty
                        document.querySelector('.product_quantity_wrapper input').dispatchEvent(event);
                    }
                    this.parentNode.parentNode.classList.remove('inc_active') // Make Current Active
            ...
            // 		let qtyList = document.querySelectorAll('.inc_qtybundle_options_block.inc_qtybundle_options')
            // 		for(let j=0; j < qtyList.length; j++){
            // 			if(qtyList[j].getAttribute("qty") == qty11){

            // 				qtyList[j].classList.add("inc_active2")
            // 				qtyList[j].click();
            // 			}
            // 		}
            // 	}
        ...
        // Close Button    
        let closeButton = get_html_element('div', ['inc_qtybundle_close'])

        // Or Text
        let orText = get_html_element('div', ['inc_product_qty_or'])
        orText.innerText = "or"

        // Add Button
        let add_button = get_html_element('div', ['inc_qtybundle_addButton'])
        add_button.innerText = "ADD"

        // Action Button
        let select_action = get_html_element('div', ['inc_qtybundle_select'])
        select_action.innerText = "Select Quantity"

        // Move Price
        let price_parent = get_html_element('div', ['inc_product_price_parent'])
        let price_parent_label = get_html_element('div', ['inc_product_price_label'])
        if (blockType == 'showcase' && price_sibling) {
            price_parent_label.innerText = "Select a Quantity"
            price_parent.appendChild(price_parent_label)
            price_parent.appendChild(price_sibling)
        }

        // Appending In Reverse

        if (blockType == 'showcase') {
            select.appendChild(select_option_parent)
            parent.appendChild(closeButton)
            parent.appendChild(price_parent)
            parent.appendChild(orText)
            parent.appendChild(select)
            parent.appendChild(add_button)
            sourceBlock.appendChild(parent)
            sourceBlock.appendChild(select_action)
        } else if (blockType == "modal") {
            select.appendChild(select_option_parent)
            parent.appendChild(select)
            sourceBlock.appendChild(parent)
        } else {
            inc_qty_bundle_selected_parent.appendChild(select_option_parent)
            parent.appendChild(select)
            qty_bundling_block.appendChild(parent)
            var elqtyopt = inc_qty_bundle_selected_parent.querySelectorAll('.inc_qtybundle_options_block.inc_qtybundle_options')
            for(var p =0;p<elqtyopt.length;p++){
                elqtyopt[p].addEventListener('click',function(){
                    if(onloadpdp == true){
                        var qtybundle=this.getAttribute('qty')
                        INC.methods.trackingEvents("bundleProductClickTracking", {
                            "product_id": INC.dataStore.pdpProductId,
                            "core_product_id": INC.dataStore.pdpProductId
                        }, "110",null,qtybundle);
            ...
            sourceBlock.parentNode.insertBefore(qty_bundling_block, sourceBlock)
        }

        // Event Listener
        // To Open Modal
        select_action.addEventListener('click', function () {
            openModal(parent)
            sourceBlock.classList.add('inc_visible')
        })

        // To Close Modal
        add_button.addEventListener('click', function () {
            closeModal(parent)
            sourceBlock.classList.remove('inc_visible')
        })
        ...
        function openModal(parent) {
            parent.classList.remove('inc_hide')
            if (window.innerWidth < 820) {
                document.querySelector("html").classList.add("inc_overlay")
                parent.parentNode.parentNode.parentNode.parentNode.classList.add("show_overlay")
            }

        }
        function closeModal(parent) {

            parent.classList.add('inc_hide')
            ...
            if (window.innerWidth < 820) {
                if (INC.config.pageType == "cartPage") {
                    ...
                    parent.parentNode.parentNode.parentNode.parentNode.classList.remove('show_overlay')
                }
        ...
        // operative function
        // Update Product Block & QTY
        function update_prices_for_qty_bundling(productBlock, qty, price, blockType, activated) {
            ...
            var pprice = price
            var qtyInput = productBlock.querySelector('.inc_product_qty_input_text input')

            if (main_id == null) {
                var main_id = productBlock.parentElement.getAttribute('data-main_id');
                var bundle_id = productBlock.parentElement.getAttribute('data-bundle_id');
            }
            ...
            // Update Active Text IF Hasn't

            if (!productBlock.querySelector('.inc_qtybundle_options_block[qty="' + qty + '"]').classList.contains('inc_active') && !activated) {
                productBlock.querySelector('.inc_qtybundle_options_block[qty="' + qty + '"]').click()
            }
            ...
            // Update Pack Details
            if (productBlock.querySelector('.inc_qtybundle_select')) {
                productBlock.querySelector('.inc_qtybundle_select').innerText = "Pack of " + qty
            }

            // If Modal
            if (blockType == "showcase" || blockType == "modal") {
                productBlock.querySelector('.inc_qtybundle_options_title').innerHTML = "Select a Pack: " + "<span>Pack of " + qty + "</span>"
            }

            // Update Active Price
            // productBlock.querySelector('.inc_product_desc_ecirp_active_text').innerText = "NOW " + price

            // Hide Was & Save Price
            productBlock.querySelector('.inc_product_desc_ecirp_regular_block').style.display = "none"


            if (Object.keys(INC.dataStore.bundleCartProducts).length != 0 && productBlock.querySelector('.inc_product_desc_add_block.Inc_checked')) {
                INC.methods.updateBundleCart({
                    sku: id,
                    qty: qty,
                }, "add", main_id, bundle_id, pname, pprice, pack_of);
            }
        }
    ...
    INC.methods.createProductShowcaseBlock = function (pdpProductId, blockType, _bundleProductListCheck, _parentblock, elProductShowcaseBlock, mainprod) {
        ...
        let elQtyBundlingBlock = productBlock.querySelector(".inc_product_qty_bundling_block")
            ...
            if (document.querySelector('.inc_sidebar_modal_block.inc_active') == null) {
                ...
                if (elAddText.textContent == "Add") {
                    if (prodflag == trueflag) {
                        ...
                        let pack_of = null
                        if (modalProductBlock.querySelector('.inc_qtybundle_options_block.inc_active')) {
                            pack_of = modalProductBlock.querySelector('.inc_qtybundle_options_block.inc_active').getAttribute('qty')
                        }
                        if (INC.config.pageType == "pdp" && blockType == "pdp") {
                            INC.methods.updateBundleCart({
                                sku: sku,
        ...
        if (productObj.QuantityBasedBundle) {
            create_qty_bundling_block(productObj.QuantityBasedBundle, productBlock, elQtyBundlingBlock, price_sibling, 'showcase', productObj)
            if (mainprod == "mainprod") {

                let toBeAppended = document.querySelector('.product_quantity_wrapper')

                create_qty_bundling_block(productObj.QuantityBasedBundle, productBlock, toBeAppended, price_sibling, 'client', productObj)

                // Handle Client Packs
                if (document.querySelector('.product_quantity_wrapper input')) {
                    document.querySelector('.product_quantity_wrapper input').addEventListener('keydown', debounce(function (e) {
                        let qty = e.target.value
                        let selected = document.querySelector('.inc_client_bundling .inc_qtybundle_options_block[qty="' + qty + '"]')

                        if (selected) {

                            selected.click()
                        } else {
                            let preselected = document.querySelector('.inc_client_bundling .inc_qtybundle_options_block.inc_active')
                            if (preselected) {
                                preselected.classList.remove('inc_active')
                            }
                        }
                ...
                let inc_dec_buttons = document.querySelector('.product_quantity_wrapper input')
                if (inc_dec_buttons) {

                    //inc_dec_buttons.forEach(function (element) {
                    inc_dec_buttons.addEventListener('click', function (e) {

                        let qty = document.querySelector('.product_quantity_wrapper input').value
                        let selected = document.querySelector('.inc_client_bundling .inc_qtybundle_options_block[qty="' + qty + '"]')
    ...
    INC.methods.createProductModalBlock = function (pdpProductId, blockType, _bundleProductListCheck, _parentblock, elmodalblock, mainprod, rec_type) {
        ...
        if (productObj.QuantityBasedBundle) {
            let price_sibling = null
            let elQtyBundlingBlock = elAttBlock

            create_qty_bundling_block(productObj.QuantityBasedBundle, productBlock, elQtyBundlingBlock, price_sibling, 'modal', productObj)
        }
    ...
    INC.methods.updateProductBlock = function (productBlockel, findObj, blockType) {
        ...
        if (Object.keys(productObj).length != 0) {
            ...
            let blockTypeThis = null
            if (productBlock.parentElement.classList.contains('inc_product_showcase_block')) {
                blockTypeThis = "showcase"
            } else if (productBlock.parentElement.classList.contains('inc_product_modal_block')) {
                blockTypeThis = "modal"
            }
            // PRODUCT BUNDLING
            if (productObj.QuantityBasedBundle) {
                ...
                let elQtyBundlingBlock = null
                let price_sibling = productBlock.querySelector('.inc_product_qty_block')
                if (blockTypeThis == 'showcase') {
                    elQtyBundlingBlock = productBlock.querySelector(".inc_product_qty_bundling_block")
                    elQtyBundlingBlock.innerHTML = ""
                    elQtyBundlingBlock.style.display = "block"
                } else {
                    elQtyBundlingBlock = productBlock.querySelector(".inc_product_desc_att_block")
                    if (productBlock.querySelector(".inc_qtybundle")) {
                        productBlock.querySelector(".inc_qtybundle").remove()
                    }
                }
                if (elQtyBundlingBlock) {
                    create_qty_bundling_block(productObj.QuantityBasedBundle, productBlock, elQtyBundlingBlock, price_sibling, blockTypeThis, productObj)


                    // For Client
                    if (productBlock.parentElement.parentElement.parentElement.classList.contains('inc_pdp_product-main_block')) {
                        document.querySelectorAll('.inc_product_qty_bundling_block.inc_client_bundling').forEach(function (element) {
                            element.remove()
                        })
                        let toBeAppended = document.querySelector('.product_quantity_wrapper')
                ...
                if (document.querySelector('.inc_product_qty_bundling_block.inc_client_bundling') != null) {
                    if (productBlock.parentElement.parentElement.parentElement.classList.contains('inc_pdp_product-main_block')) {
                        document.querySelectorAll('.inc_product_qty_bundling_block.inc_client_bundling').forEach(function (element) {
                            element.remove()
                        })
                ...
                if (blockTypeThis == 'showcase') {
                    elQtyBundlingBlock = productBlock.querySelector(".inc_product_qty_bundling_block")
                    elQtyBundlingBlock.style.display = "none"
                } else {
                    elQtyBundlingBlock = productBlock.querySelector(".inc_product_desc_att_block")
                    if (productBlock.querySelector(".inc_qtybundle")) {
                        productBlock.querySelector(".inc_qtybundle").remove()
    ...
    INC.methods.updateBundleCart = function (bundleCartObj, type, main_id, bundle_id, pname, pprice, pack_of) {
        let dataStore = INC.dataStore;
        let listToDelete = []
        if(bundleCartObj.sku != null){
            if (!Object.prototype.hasOwnProperty.call(dataStore, "bundleCartProducts")) {
                dataStore['bundleCartProducts'] = {};
                INC.config.pdpaddedProductList = []
                INC.config.bundle_tracking = []
            ...
            switch (type) {
                case "add":
                    ...
                    if (INC.config.pdpaddedProductList.indexOf(main_id) == -1) {
                        ...
                        INC.config.bundle_tracking[INC.config.pdpaddedProductList.indexOf(main_id)].pack_of = pack_of
                        INC.config.bundle_tracking[INC.config.pdpaddedProductList.indexOf(main_id)].qty = bundleCartObj["qty"]
                    }
                    break;
                case "sub":
                    ...
                    for (let childProductId in productListObj[mainProductId]) {
                        if (sku == productListObj[mainProductId][childProductId]["sku"]) {
                            if (duplicate_check_produ.indexOf(productListObj[mainProductId][childProductId]["sku"]) == -1) {
                                ...
                                if (productListObj[mainProductId][childProductId].QuantityBasedBundle !== '' && productListObj[mainProductId][childProductId].QuantityBasedBundle !== null) {
                                    let getQtyPrice = prepareqty_bundles(productListObj[mainProductId][childProductId].QuantityBasedBundle, bundleCartProducts[sku], activePrice)
                                    activePrice = getQtyPrice.activePrice.replace('£', '')
                                }
    ...
    INC.methods.trackingEvents = function (eventType, eventDataObj, pageType, rec_type,qtybundle) {
        ...
        switch (eventType) {
            ...
            case "bundleProductClickTracking":
                ...
                if(qtybundle != undefined && pageType == '110'){
                    importEventDataObj["qty"] =qtybundle;
                }
    ...
    function clickListener(e) {
        ...
        if (clickedElement != null) {
            try {
                ...
                if (clickedElement.className == 'inc_product_block configurable show_overlay' || clickedElement.classList.contains("show_overlay")) {
                    clickedElement.classList.remove('show_overlay')
                    if (clickedElement.querySelector('.inc_product_qty_bundling_block').classList.contains('inc_visible')) {
                        clickedElement.querySelector('.inc_product_qty_bundling_block').classList.remove('inc_visible')
                        clickedElement.querySelector('.inc_product_qty_bundling_block .inc_qtybundle').classList.add('inc_hide')
                    }
                }
                if (document.querySelector('.inc_product_modal_block.inc_active') != null) {
                ...
                if (clickedElement != null) {
                    ...
                    if ((document.querySelector('.inc_qtybundle_options') && onloadpdp) || INC.config.pageType == "cartPage" || INC.config.pageType == "pdp") {
                        if (clickedElement.parentNode != null) {
                            if (clickedElement.parentNode.parentNode != null) {
                                if (!clickedElement.classList.contains('inc_qtybundle') && !clickedElement.classList.contains('inc_qtybundle_close') && !clickedElement
                                    .classList.contains('inc_product_price_parent') && !clickedElement.classList.contains('inc_product_price_label') && !clickedElement
                                        .classList.contains('inc_product_qty_block singleqty') && !clickedElement.classList.contains('inc_product_qty_down_block') && !
                                    clickedElement.classList.contains('inc_product_qty_down_btn_block') && !clickedElement.classList.contains(
                                        'inc_product_qty_down_btn') && !clickedElement.classList.contains('inc_product_qty_input_block') && !clickedElement.classList
                                            .contains('inc_product_qty_input_text_block') && !clickedElement.classList.contains('inc_product_qty_input_text') && !clickedElement
                                                .classList.contains('inc_add_text') && !clickedElement.classList.contains('inc_product_qty_up_block') && !clickedElement.classList
                                                    .contains('inc_product_qty_up_btn_block') && !clickedElement.classList.contains('inc_product_qty_up_btn') && !clickedElement
                                                        .classList.contains('inc_product_qty_or') && !clickedElement.classList.contains('inc_qtybundle_options') && !clickedElement
                                                            .classList.contains('inc_qtybundle_options_individual') && !clickedElement.classList.contains('inc_qtybundle_options_block') && !
                                    clickedElement.classList.contains('inc_qtybundle_addButton') && !clickedElement.classList.contains('inc_qtybundle_select') && !
                                    clickedElement.classList.contains('inc_qtybundle_options_title') && !clickedElement.parentElement.parentElement.classList.contains(
                                        'inc_qtybundle_options') && !clickedElement.parentElement.classList.contains('inc_qtybundle_options') && !clickedElement
                                            .parentElement.parentElement.classList.contains('inc_qtybundle_options_individual') && !clickedElement.parentElement.parentElement.classList.contains('inc_qtybundle_options_block') && !clickedElement.parentElement.classList.contains('inc_product_qty_input_text')) {


                                    let allParents = document.querySelectorAll('.inc_pdp_block .inc_qtybundle')
                                    let allParentsBlock = document.querySelectorAll('.inc_af_block .inc_qtybundle')
                                    let allParentSidebar = document.querySelectorAll('.sidebar_outer .inc_qtybundle')
                                    let allParentscart = document.querySelectorAll('.inc_af_block .inc_qtybundle')

                                    if (document.querySelector('.inc_product_qty_bundling_block.inc_visible')) {

                                        document.querySelector('.inc_product_qty_bundling_block.inc_visible').classList.remove('inc_visible')
                                    }
                                    allParents.forEach((ele) => {
                                        if (!ele.classList.contains('inc_hide') && !ele.classList.contains('inc_qtybundle_modal')) {
                                            ele.classList.add('inc_hide')
                                            document.querySelector("html").classList.remove("inc_overlay")
                                        }
                                    })
                                    allParentsBlock.forEach((ele) => {
                                        if (!ele.classList.contains('inc_hide') && !ele.classList.contains('inc_qtybundle_modal')) {
                                            ele.classList.add('inc_hide')
                                            document.querySelector("html").classList.remove("inc_overlay")
                                        }
                                    })
                                    allParentSidebar.forEach((ele) => {
                                        if (!ele.classList.contains('inc_hide') && !ele.classList.contains('inc_qtybundle_modal')) {
                                            ele.classList.add('inc_hide')
                                            //document.querySelector("html").classList.remove("inc_overlay")
                                        }
                                    })
                                    allParentscart.forEach((ele) => {
                                        if (!ele.classList.contains('inc_hide') && !ele.classList.contains('inc_qtybundle_modal')) {
                                            ele.classList.add('inc_hide')
                                            document.querySelector("html").classList.remove("inc_overlay")
                                        }
                                    ...
                                    if (clickedElement.classList.contains("inc_product_qty_bundling_block")) {
                                        if (INC.config.pageType != "cartPage") {
                                            if (window.innerWidth < 820) {
                                                window.scrollTo(0, 1000);
                                            } else {
